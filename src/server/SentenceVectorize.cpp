...
void WordsVectorize::FindsNumOfWordsInDatasetSentences(const std::vector<std::string>& methods, const std::vector<std::string>& pages,
	...
	MapIndexInit(numOfUniqueWordsInSentenceInDataset, methods);
	MapIndexInit(numOfUniqueWordsInSentenceInDataset, pages);
	MapIndexInit(numOfUniqueWordsInSentenceInDataset, protocol);
	MapIndexInit(numOfUniqueWordsInSentenceInDataset, agent);
	MapIndexInit(numOfUniqueWordsInSentenceInDataset, SQL);
	MapIndexInit(numOfUniqueWordsInSentenceInDataset, XSS);
	MapIndexInit(numOfUniqueWordsInSentenceInDataset, Win);
	...
	for (const auto& sentence : dataset) {
		...
		for (auto& w : unique) {
			numOfUniqueWordsInSentenceInDataset[w]++;
		}
	}
}
...
void WordsVectorize::calculIDF() {
	int i = 0;
	idf = new WordsIDF[numOfUniqueWordsInSentenceInDataset.size()];
	for (const auto& words : numOfUniqueWordsInSentenceInDataset) {
		int wordSize = words.first.size() + 1;
		idf[i].word = new char[wordSize];
		FromStringToChar(words.first, idf[i].word, wordSize - 1);
